...
class ClosureTableTreeTest extends AbstractDatabaseTestCase
{
    private const string DATA_DIR = __DIR__ . '/../../data';
    private const string NODES_CSV_PATH = self::DATA_DIR . '/treeoflife_nodes.csv';
    private const string LINKS_CSV_PATH = self::DATA_DIR . '/treeoflife_links.csv';
    ...
    protected function setUp(): void
    {
        ...
        $this->getConnection()->execute('DELETE FROM tree_of_life.tree_of_life_closure_table WHERE true');
        $this->getConnection()->execute('DELETE FROM tree_of_life.tree_of_life_node WHERE true');

        $this->service = new ClosureTableTreeService($this->getConnection());
    }
    ...
    private function assertEqualTrees(TreeOfLifeNode $expected, TreeOfLifeNode $root): void
    {
        ...
        {
            $childIndex = null;
            for ($j = 0, $jMax = count($children); $j < $iMax; ++$j)
            {
                if ($expectedChildren[$i]->getId() == $children[$j]->getId())
                {
                    $childIndex = $j;
                    break;
                }
            }
            self::assertTrue(!is_null($childIndex));
            $this->assertEqualTrees($expectedChildren[$i], $children[$childIndex]);
        }
    }
